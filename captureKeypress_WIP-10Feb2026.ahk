...
buildKeyCombos(keys) {
	...
	loop maxLen {
		...
		combo := ""
		loop len {
			entry := keys[keyCount - len + A_Index]
			combo .= NormalizeChar(entry.char)
		}
		combos[len] := combo
	}
	return combos
...
ShowNvimTreeWarning() {
	...
	x := winX + (winW - width) // 2
	y := winY + (winH - height) // 2

	NvimTreeGui.Show("x" x " y" y " w" width " h" height " NoActivate")
...
LogKey(char, *) {
	...
	if (keys.Length >= 2) {
		...
		if (hasMatchingKeybind(combos, gateEntryBindings) && currentMode = "NORMAL") {
			gateActive := true
			return
		}
	}
...
FlushKeys() {
	...
	for k in keys {
		...
		FileAppend(k.time " | " k.char " | " k.mode " | " k.gate "`n", logFile)
	}
