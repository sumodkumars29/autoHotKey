...
; MOTION_COMPLETIONS := Map(
;   "i", Map("w",1,"W",1,"b",1,"B",1,"(",1,"{",1,"[",1,"<",1,"`"",1,"'",1,"``",1),
;   "a", Map("w",1,"W",1,"b",1,"B",1,"(",1,"{",1,"[",1,"<",1,"`"",1,"'",1,"``",1)
; )
; ===============================================
...
HandleNormalToInsert(key) {
  global currentMode, pendingOperator, pendingCount, pendingMotion
  ...
  if (pendingMotion != "") {
    if (pendingOperator = "c") {
      ...
      } else if (pendingMotion = 'f' || pendingMotion = 'F' || pendingMotion = 't' or pendingMotion = 'T'){
        ; for f, F, t and T
        SetMode("INSERT")
        resetPendingTrackers()
        return FALSE
